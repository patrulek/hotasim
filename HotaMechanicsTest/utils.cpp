...
namespace TestUtils {
	...
	const std::vector<UnitStack> createArmy(const std::string& tmp1, const int16_t size1,
		...
		std::vector<UnitStack> army; army.reserve(7);
		army.push_back(UnitStack{ &unit_templates()[tmp1], size1 });
		if (size2 == 0) return army;

		army.push_back(UnitStack{ &unit_templates()[tmp2], size2 });
		if (size3 == 0) return army;

		army.push_back(UnitStack{ &unit_templates()[tmp3], size3 });
		if (size4 == 0) return army;

		army.push_back(UnitStack{ &unit_templates()[tmp4], size4 });
		if (size5 == 0) return army;

		army.push_back(UnitStack{ &unit_templates()[tmp5], size5 });
		if (size6 == 0) return army;

		army.push_back(UnitStack{ &unit_templates()[tmp6], size6 });
		if (size7 == 0) return army;

		army.push_back(UnitStack{ &unit_templates()[tmp7], size7 });
		return army;
	}
	...
	CombatHero createHero(const std::vector<UnitStack>& _army, const CombatSide _side, const int _atk, const int _def) {
		...
		ArmyPermutation* perm = new ArmyPermutation();
		*perm = Utils::createBaseArmyPermutation(const_cast<std::vector<UnitStack>&>(_army));

		return CombatHero(*tmp, *perm, _side);
	}
