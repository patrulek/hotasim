...
namespace CombatAITest {
	...
	//	auto value = ai.getAttackableHexesForUnit(active_stack);
	//	value[value[FIELD_SIZE]++] = getHexId(5, 5);
	//	std::sort(std::begin(expected), std::begin(expected) + expected[FIELD_SIZE]);
	//	std::sort(std::begin(value), std::begin(value) + value[FIELD_SIZE]);
	//	EXPECT_EQ(expected, value);
